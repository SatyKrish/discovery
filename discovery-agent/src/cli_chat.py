...
PENDING_STATES = {"queued", "running", "in_progress", "pending", "processing", "starting"}
...
def _extract_status_url(api: str, first_reply: Dict[str, Any], session_id: Optional[str]) -> Optional[str]:
    ...
    if session_id:
        return f"{api.rstrip('/')}/status/{session_id}"
...
def _poll_until_done(status_url: str, poll_interval: float = 1.0, timeout: float = 120.0) -> Tuple[Optional[str], Dict[str, Any]]:
    ...
    while True:
        ...
        if state in DONE_STATES:
            # Done, even if no out (return whatever we have)
            return out, last
...
def main() -> None:
    ...
    try:
        while True:
            ...
            # Try to extract a ready answer
            text = _extract_output_text(reply)
            if text:
                _print(f"Agent: {text}")
                continue

            # If server says "see status endpoint", follow it
            ...
            if status_url:
                ...
                if out:
                    ...
                    # still print something useful for debugging
                ...
                # Fall back: show whole reply (likely includes hint fields you can map)
